...
num_active_client=5
num_class = 10
is_iid = True # STL10 do not have label for unlabeled data
is_bias=True
l2_factor=1e-4
...
def get_dataset():
    ...
    for i in range(10):
        ...
        val_labels.append(labels[np.where(labels == 1)[1]==i][1000:1150])
        test_dataset.append(images[np.where(labels == 1)[1]==i][1150:, :, :, :])
        test_labels.append(labels[np.where(labels == 1)[1]==i][1150:])
    val_images = np.concatenate(val_dataset, axis=0)
    val_labels = np.concatenate(val_labels, axis=0)
    test_images = np.concatenate(test_dataset, axis=0)
    test_labels = np.concatenate(test_labels, axis=0)

    return train_dataset, val_images, val_labels, test_images, test_labels, unlabeled_images
...
train_dataset, val_images, val_labels, test_images, test_labels, unlabeled_images = get_dataset()
data_distribution = get_data_distribution(is_iid)
train_images_list, train_labels_list, train_unlabels_list = get_client_dataset(dataset=train_dataset, unlabeled_dataset=unlabeled_images, ratio=data_distribution)
...
def get_model(model_arch='resnet8'):
    def conv_block(out_channels, pool=False, pool_no=2):
        layers = [tf_layers.Conv2D(out_channels, kernel_size=(3, 3), padding='same', use_bias=True, strides=(1, 1),
                                        kernel_initializer=tf_initializers.VarianceScaling(),  kernel_regularizer=tf_regularizers.l2(l2_factor)),
                        tf.keras.layers.ReLU()]
        if pool: layers.append(tf_layers.MaxPooling2D(pool_size=(pool_no, pool_no)))
        return tf_models.Sequential(layers)
    inputs = tf_keras.Input(shape=(96,96,3))
    ...
    out = conv_block(512, pool=True, pool_no=4)(out)
...
class Client:
    ...
    def training(self, client_images, client_labels, client_unlabels, client_model, global_model_weights, local_epoch, batch_size):
        ...
        for e in range(local_epoch):
            ...
            idx = np.random.permutation(len(client_unlabels))
            client_unlabels = client_unlabels[idx]
